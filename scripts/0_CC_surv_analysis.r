# Script: 0_CC_surv_analysis.r
# Title: Climate chamber germination and survival analysis
# Manuscript:
...
# Authors: Katalin Csillery, with input from Johannes Alt, Mert Celik, Marjorie Bison
# Created: 2026-01-28
# Last updated: 2026-02-07
##
# Inputs: climate_chamber_pheno_stages_seeds.csv, seed_provenance_details.csv
# Outputs: CC_rawdata.pdf and derived germination metrics tables
##
# Notes:
...
dat <- merge(dat, seed.dat, by=c("Provenance", "Species"), all.x=TRUE)
...
# write.table(merge(mycols, seed.dat, by="ID", all=TRUE), file="../mycols.txt")
...
dat2 <- merge(dat.sub, seed.sum.cmb, by=c("ID", "Date", "Moisture_treatment"), all=TRUE)
dat <- rbind(dat, dat2)
...
res <- merge(res, res3, all=TRUE)
res <- merge(res, res4, all=TRUE)

# combine with seed data
res <- merge(res, seed.dat, by=c("ID", "Genus"), all.x=TRUE)
...
load("mm_dat.RData")
all.res <- merge(res, mm.dat[c("ID", "g_MLE", "gamma_MLE", "delta_MLE", "alpha_MLE", "beta_MLE")], by="ID", all=TRUE)
...
all.res.sub$Moisture_treatment = factor(all.res.sub$Moisture_treatment, ordered=TRUE, levels=c("moist", "dry"))
...
datsurv = merge(datsurv, seed1, by="Provenance", all.x=TRUE)
...
# merge with seed data
seed1 = read.csv("seed_provenance_details.csv")
datsurv = merge(datsurv, seed1, by=c("Species", "Provenance"), all.x=TRUE)
