# Script: 00_plot.r
# Title: Plotting and helper functions for Markov model outputs
# Manuscript:
...
# Authors: Madleina Caduff, with slight modifications by Katalin Csillery
# Created: 2026-01-28
# Last updated: 2026-02-07
##
# Inputs: Text trace files from Markov model runs
# Outputs: Helper data frames and utility functions used by downstream scripts
##
# Notes:
...
read_MLE <- function(all_files, species){
  ...
  for (c in unique(all_files$class)){
    ...
    if (length(name) == 0 || file.info(paste0(path, name))$size == 0){
      ...
      f <- read.table(paste0(path, name), header = TRUE)
      f <- f[nrow(f),]
...
guess_germination_without_death <- function(MLE, species){
  obs_data <- read.table(paste0("../data/Input_obs_", species, ".txt"), header = TRUE)
  ...
  # coordinates for scale bar
    ...
    for (i in 1:(ncols-1)) {
        rect(scale_x[i], scale_y, scale_x[i+1], scale_y - 0.7, 
             col = cols[i], border = cols[i], xpd = TRUE)
    }
    ...
    axis(1, at = at,
         labels = labels,
         line = scale_y, tick = TRUE, xpd = TRUE, outer = TRUE, col = NA, col.ticks = NA)
}
...
makeImageWithMeans <- function(data, stat = mean, addYLabels = TRUE){
    ...
    for(i in 1:length(xlabs)){
        for (j in 1:length(ylabs)){
            ...
            addPolygon(i, j, col = "lightgrey", hatched = TRUE)
        }
    }
    ...
    for(i in 1:nrow(data)){
        if (data$never_germinated[i]){ 
            x <- which(xlabs == data[i,1])
            y <- which(ylabs == data[i,2])
            addPolygon(x, y, col = "lightgrey", crossed = TRUE)
        }
    }
...
# add box
    ...
    coltoalpha = ifelse(is.na(mean(data[data[,1] == xlabs[i], 3], na.rm=TRUE)), "#00000000", "black")

# add stat to edges
    for(i in 1:length(xlabs)){

        addPolygon(i, length(ylabs) + 1.2, col = alpha(coltoalpha, mean(data[data[,1] == xlabs[i], 3], na.rm=TRUE)))
    }
    for(j in 1:length(ylabs)){
        addPolygon(length(xlabs) + 1.2, j, col = alpha(coltoalpha, mean(data[data[,2] == ylabs[j], 3], na.rm=TRUE)))
    }
